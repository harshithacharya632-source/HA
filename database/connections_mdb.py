...
async def add_connection(group_id, user_id):
    ...
    if mycol.count_documents( {"_id": user_id} ) == 0 and mycol.count_documents( {"_id": int(user_id)} ) == 0:
        try:
        ...
        try:
            existing = mycol.find_one({"_id": user_id}) or mycol.find_one({"_id": int(user_id)})
            mycol.update_one(
                {'_id': existing['_id']},
                {
